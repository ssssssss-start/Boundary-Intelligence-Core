...
LOG_SENSITIVE_REDACTION = os.getenv("ANTI_FRAUD_LOG_SENSITIVE_REDACTION", "1").lower() in {"1", "true", "yes", "on"}
...
def init_logger():
    ...
    if LOG_FILE_ENABLE:
        ...
        logger.add(
            ...
            enqueue=True,
            backtrace=True,
            diagnose=os.getenv("ANTI_FRAUD_LOG_DIAGNOSE", "0").lower() in {"1", "true", "yes", "on"}
        )
...
def redact_log_record(record):
    """Prevent credentials in ad-hoc log messages from reaching sinks."""

    if LOG_SENSITIVE_REDACTION:
        # Import lazily: memory_manager itself uses this logger and importing
        # the memory package during module initialization would create a cycle.
        from app.query_process.agent.memory.memory_redactor import redact_sensitive_text

        record["message"] = redact_sensitive_text(str(record.get("message", "")))
    return record

# 应用终极修复，导出全局可用的logger
logger = base_logger.patch(fix_log_position).patch(redact_log_record)
